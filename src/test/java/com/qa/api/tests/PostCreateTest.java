...
public class PostCreateTest extends BaseTest {
	...
	@Test
	public void createTestApiwithLombak() {

		User usera = new User(null,"aaSudheer", StringUtils.getRandomEmailID(), "male", "active");

		Response response = restClient.post(BASE_URL_GOREST,"/public/v2/users", usera, null, null, AuthType.BEARER_TOKEN,
				ContentType.JSON);
		...
		//GET
		Response resGet= restClient.get(BASE_URL_GOREST,"/public/v2/users/"+userId, null, null, AuthType.BEARER_TOKEN, ContentType.JSON);
	...
	@Test
	public void createTestAPIWithbuilderPat() {
		
		
		User user = User.builder().name("Myra").email(StringUtils.getRandomEmailID()).gender("Female").status("active").build();
